...
public class BurpExtender implements IBurpExtender,IScannerCheck,ITab{
    ...
    public void registerExtenderCallbacks(IBurpExtenderCallbacks callbacks) {
        ...
        stdout.println("                author: shifeige             ");
        stdout.println(" GitHub:https://github.com/shifeige/burpscan ");
        stdout.println("=============================================");
    ...
    public List<IScanIssue> doPassiveScan(IHttpRequestResponse baseRequestResponse) {
        ...
        if(YamlReader.getInstance(callbacks).getBoolean("scanModule.HadoopScan.isStart")){
            List<IHttpRequestResponse>  HadoopScanResults= HadoopScan.ScanMain(baseRequestResponse,callbacks,helpers, stdout);
            for(IHttpRequestResponse result:HadoopScanResults){
                if(result != null){
                    String payload = helpers.analyzeRequest(result).getUrl().getPath();
                    if(payload.endsWith("/cluster")){
                        Addissuse(result,"Hadoop Foud!",payload);
                    }
                }
            }
        }
